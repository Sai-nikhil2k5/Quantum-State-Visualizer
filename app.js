...
if (options.purity && options.purity < 0.95) {
 ...
 createVectorArrow(blochVector, options) {
  // Map Bloch coordinates to scene coordinates
  const mapped = new THREE.Vector3(blochVector.x, blochVector.z, -blochVector.y);
  const length = mapped.length();
  if (length < 0.01) return;

  const dir = mapped.clone().normalize();

  // Adjust arrow dimensions
  const headLen = Math.min(0.25, length * 0.35);  // Arrowhead length
  const headRadius = headLen * 0.45;              // Arrowhead radius
  const shaftRadius = Math.max(0.03, headRadius * 0.4); // Shaft radius
  const shaftLen = Math.max(0.0001, length - headLen);  // Shaft length

  // ✅ Use a constant yellow color for all arrows
  ...
  // Shaft
  const shaftGeom = new THREE.CylinderGeometry(shaftRadius, shaftRadius, shaftLen, 32);
  const shaft = new THREE.Mesh(shaftGeom, material);
  shaft.position.y = shaftLen / 2;
  group.add(shaft);

  // Head
  const headGeom = new THREE.ConeGeometry(headRadius, headLen, 32);
  const head = new THREE.Mesh(headGeom, material);
  head.position.y = shaftLen + headLen / 2;
  group.add(head);

  // Rotate the entire arrow from +Y to the Bloch vector direction
  const quat = new THREE.Quaternion().setFromUnitVectors(
    new THREE.Vector3(0, 1, 0), // start along +Y
    dir                         // rotate to Bloch vector
  );
  group.quaternion.copy(quat);

  // Save & add to scene
  this.blochVector = group;
